...
ADD CONSTRAINT IF NOT EXISTS valid_participant_email 
CHECK (participant_email ~* '^[^@\s]+@[^@\s]+\.[^@\s]+$');

ALTER TABLE public.registrations 
ADD CONSTRAINT IF NOT EXISTS valid_participant_name_length 
CHECK (length(trim(participant_name)) >= 2 AND length(trim(participant_name)) <= 100);

-- Add audit trigger for profile changes (only for INSERT/UPDATE/DELETE operations)
...
BEGIN
  ...
  PERFORM public.log_security_event(
    ...
    CASE 
      ...
      WHEN TG_OP = 'UPDATE' AND OLD.role != NEW.role THEN 50 
